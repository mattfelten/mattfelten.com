...
1. STARTING POINT - Show the chaos/before state
   Option A: Component audit artboard (shows the scale of inconsistency)
   Option B: Original Sketch file (shows humble beginnings)
   → Recommend: Component audit. It's more visceral - seeing dozens of
   inconsistent UI patterns makes the problem tangible.

2. BUILDING THE FOUNDATION - Show the methodology and output
   - Process diagram (how you worked)
   - Docs landing page (what you shipped)
   → Both are worth keeping. Process shows the thinking, docs shows the result.

3. MIGRATING TO FIGMA - Currently has button examples
   → Cut this section's images entirely. The text about 0.16% detach rate
   and 10k variations is more compelling than a button screenshot.

4. COMPONENT DEVELOPMENT - Show design thinking, not final output
   Option A: Component proposal (Input Addons)
   Option B: Explorations artboard (zoomed out variations)
   → Recommend: Explorations artboard. It shows the messy thinking process,
   which is more interesting than a polished spec doc.

5. OUTCOMES - Show the communication/adoption work
   Option A: Release announcement email
   Option B: New feature slide for company meetings
   → Either works. The email shows scale (sent to entire org).
   The slide shows internal marketing.

IMAGES TO SKIP:
- Button Figma/docs/code — not interesting
- ServiceTitan homepage — not relevant
- Contribution guide — could be text if you want to include it
- Scoped Search prototype — too specific, doesn't support main narrative
*/}
...
{/* No image here - the stats (0.16% detach rate, 10k variations) are more compelling than component screenshots */}
...
{/* TODO: Explorations artboard - shows the messy design thinking process, more interesting than polished specs */}
